...
namespace DftMosaic.Cmd
{
    class Program
    {
        ...
        private static List<MosaicRequestArea> mosaicAreas = new List<MosaicRequestArea>();
        ...
        {
            Console.WriteLine("usage: DftMosaic.Cmd.exe -m X,Y,W,H[,A][;X2,Y2,W2,H2...]");
            Console.WriteLine("       [-h | --help] [-o | --output <output image>");
            Console.WriteLine("       [-t | --type <type>]");
            Console.WriteLine("       <input image>");
            Console.WriteLine();
            Console.WriteLine("    X,Y,W,H,A: Specify the area to be mosaiced. No space is allowed for before and after the comma.");
            ...
            Console.WriteLine("      A: An integer value. Angle of the mosaiced area.");
            ...
            try
            {
                for (int i = 0; i < args.Length; i++)
                {
                    switch (args[i])
                    {
                        ...
                        case "-m" or "--mosaic":
                            var mOption = args[++i];
                            var regex = new Regex(@"(?<x>\d+),(?<y>\d+),(?<w>\d+),(?<h>\d+)(?:,(?<a>\d+))?;?");
                            ...
                            foreach (Match match in matches)
                            {
                                ...
                                mosaicAreas.Add(new(
                                    ...
                                    int.Parse(match.Groups["a"].Success ? match.Groups["a"].Value : "0")
                                    ));
                            }
                            break;
                        case "-o" or "--output":
